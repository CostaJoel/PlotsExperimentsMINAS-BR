...
def plot_measures_over_time_sinais(data,
                                   ...
                                   savefig=False,
                                   name_fig=None):
    sns.set_theme(style="ticks",context='notebook', font_scale=1.5)
    ...
    x = 0
    pos = [-30, -5, 20, 45, 60, 85, 70, 95, 120, 120, 110, 140, 100, 100, 100, 100, 100] #mediamill original configuration
    # pos = [-30, 30] #Scene original configuration
    ...
    axs[2].set_xlabel("Windows")
    axs[2].set_ylabel("F1M and UnkRM")
...
def barplot_precision_recall(datasets, savefig=False, name_fig="barplot_precision_recall.pdf"):
    ...
    for k,v in datasets.items():
        ...
        j += 1
        pr = v[["Classifier", "PrM"]]
        pr.columns = ["Classifier", "Precision Recall"]
        pr["metric"] = "PrM"

        re = v[["Classifier", "ReM"]]
        re.columns = ["Classifier", "Precision Recall"]
        re["metric"] = "ReM"

        data = pd.concat([pr, re])

        ax = sns.barplot(data=data, x="Classifier", y="Precision Recall", hue="metric")
        ax.set_ylim((0, 1))
        ax.set_xlabel(k, fontsize=14)
